...
if respuesta_pos.status_code == 200:
    ...
    create_table_sql = """
                      CREATE TABLE IF NOT EXISTS mxxn13_coderhouse.posiciones_premier_league (
                          ...
                          Fecha_ingesta VARCHAR,
                          PRIMARY KEY (id_eq, name_eq)
                      );
                      """.format(table_name='mxxn13_coderhouse.posiciones_premier_league')

        # Ejecutar la sentencia SQL para crear la tabla
    try:
      with conn.connect() as con:
        con.execute(create_table_sql)
      print('Tabla creada exitosamente con primary key compuesta.')
    except Exception as e:
      print('Error al crear la tabla (posiciones_premier_league):', e)
...
#-------------------------------------------------------- #-------------------------------------------------------- #--------------------------------------------------------
    ...
    create_table_sql = """
                      ...
                      status_par VARCHAR,
                      id_eq_local INT,
                      eq_local_win BOOLEAN,
                      eq_local_goles INT,
                      id_eq_visitante INT,
                      eq_visitante_win BOOLEAN,
                      eq_visitante_goles INT,
                      resultado_final_local INT,
                      resultado_final_visitante INT,
                      resultado_extratime_local INT,
                      resultado_extratime_visitante INT,
                      penales_local INT,
                      penales_visitante INT,
                      PRIMARY KEY (id_partido)
                  );
                  """.format(table_name='mxxn13_coderhouse.partidos_premier_league')
    ...
    try:
        ...
        print('Error al crear la tabla(partidos_premier_league):', e)
...
#-------------------------------------------------------- #-------------------------------------------------------- #--------------------------------------------------------
    ...
    try:
        ...
        print('Error al crear la tabla (estadios_premier_league):', e)
